...
#' Plot a Bar Plot
#'
#' Creates a Bar plot visualizing scores and their confidence intervals
...
#'   The metric can be converted to "z" and other supported metrics.
...
#' @param abbreviations (Optional) Logical value indicating whether to abbreviate
#'   test names. Defaults to FALSE.
#' @param color The color of the points and error bars. Defaults to "black".
#' @param fill The fill color of the bars. Defaults to "white".
...
#' plot_bar(df, score = score, test = test, group = group, ci.lb = ci.lb, ci.ub = ci.ub)
#'
#' # Plot with no descriptors and shading
#' plot_bar(df, score = score, test = test, group = group, ci.lb = ci.lb, ci.ub = ci.ub,
#'           descriptors = FALSE, shading = FALSE)
plot_bar <- function(data, score, metric = "index", test, group, ci.lb, ci.ub, axis.label.metric = metric,
                     color = "black",
                     fill = "white",
                     z.lines = TRUE,
                     z.line.color = "black",
                     descriptors = TRUE,
                     descriptors.color = "black",
  ...
  if (any(dup_rows)) {
    ...
    warning(glue::glue("Duplicate rows detected based on 'test' and 'group'. Rows {dup_indices} will be removed."))
    data <- data[!dup_rows, ]
  }

  # Convert to index
  data <- dplyr::mutate(data, index = neuropsytools::convert_standard({{score}}, metric = metric, metric.new = "index"))

  if (metric != "index" && !missing(ci.lb)) {
    data <- dplyr::mutate(data, ci.lb = neuropsytools::convert_standard({{ci.lb}}, metric = metric, metric.new = "index"))
  }
  if (metric != "index" && !missing(ci.ub)) {
    data <- dplyr::mutate(data, ci.ub = neuropsytools::convert_standard({{ci.ub}}, metric = metric, metric.new = "index"))
  }
  ...
  labeling_functions <- list(
    ...
    t = function(x) format(neuropsytools::convert_standard(x, metric = "index", metric.new = "t")),
    index = function(x) format(neuropsytools::convert_standard(x, metric = "index", metric.new = "index")),
    scaled = function(x) format(neuropsytools::convert_standard(x, metric = "index", metric.new = "scaled")),
    z = function(x) format(neuropsytools::convert_standard(x, metric = "index", metric.new = "z"))
  )
  ...
  # Descriptor labels
  descriptor_labels <- data.frame(index = c(62.5, 77.5, 92.5, 107.5, 122.5, 137.5), label = c("Below", "Low", "Average", "Average", "High", "Above"))
  ...
  p <- data |>
    ...
    ggplot2::ggplot(ggplot2::aes(
      x = forcats::fct_rev(test), y = index
    ))

  if (shading) {
    p <- p + ggplot2::geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 85, ymax = 115),
                                fill = shading.color, alpha = 0.2)
  }
    ...
    ggplot2::geom_col(
      color = color,
      fill = fill,
      position = position_dodge(width = 0.9)) +  # Bar color
    ggplot2::coord_flip(ylim = c(40, 150)) +
    ggplot2::facet_grid(
  ...
  if (z.lines) {
    p <- p +
      ggplot2::geom_hline(yintercept = c(70, 85, 100, 115, 130),
                          color = z.line.color, alpha = .6)
  }
  ...
  if (!missing(ci.lb) && !missing(ci.ub)) {
    p <- p +
      ggplot2::geom_errorbar(ggplot2::aes(ymin = {{ci.lb}}, ymax = {{ci.ub}}), width = .1,
                             position = ggplot2::position_dodge(.9), color = color)  # Fixed the closing parenthesis here
  }
